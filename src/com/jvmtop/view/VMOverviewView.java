...
public class VMOverviewView extends AbstractConsoleView {
	...
	public void printView() throws Exception {
		...
		for (VMInfo vmInfo : vmInfoList) {
			if (vmInfo.getState() == VMInfoState.ATTACHED) {
				...
				System.out
						.printf("%5d %-15.15s [ERROR: Could not fetch telemetries (Process DEAD?)] %n",
								vmInfo.getId(),
				...
				System.out
						.printf("%5d %-15.15s [ERROR: Connection refused/access denied] %n",
								vmInfo.getId(),
								getEntryPointClass(vmInfo.getDisplayName()));
			}
	...
	private void printVM(VMInfo vmInfo) throws Exception {
		...
		System.out
				.printf("%5d %-15.15s %5s %5s %5s %5s %5.2f%% %5.2f%% %-5.5s %8.8s %4d %2.2s%n",
						vmInfo.getId(),
						getEntryPointClass(vmInfo.getDisplayName()),
						...
						vmInfo.getThreadCount(), deadlockState);
  ...
  */
	private void printHeader() {
		System.out.printf(
				...
				"PID", "MAIN-CLASS", "HPCUR", "HPMAX", "NHCUR", "NHMAX", "CPU",
				"GC", "VM", "USERNAME", "#T", "DL");
	}

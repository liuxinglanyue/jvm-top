...
public class VMDetailView extends AbstractConsoleView {
	...
	public void printView() throws Exception {
		...
		if (command != null) {
			...
			System.out.printf(" PID %d: %s %n", vmInfo_.getId(),
					commandArray[0]);
		...
		System.out
				.printf(" UP: %-7s #THR: %-4d #THRPEAK: %-4d #THRCREATED: %-4d USER: %-12s%n",
						toHHMM(vmInfo_.getRuntimeMXBean().getUptime()), vmInfo_
								.getThreadCount(), vmInfo_.getThreadMXBean()
								.getPeakThreadCount(),
				...
				if (info != null) {
					System.out.printf(
							...
							getThreadCPUUtilization(vmInfo_.getThreadMXBean()
									.getThreadCpuTime(tid), vmInfo_
									.getProxyClient().getProcessCpuTime(), 1),
